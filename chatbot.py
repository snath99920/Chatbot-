...
#Checking the number of occurance of different words
word2count = {}
...
#Creating 2 dictionaries that map the questions words and the answers words to a unique integer
threshold = 20
questionswords2int = {}
word_number = 0
...
#Inverse dictionary of answerswords2int dictionary
answersints2word = {w_i: w for w, w_i in answerswords2int.items()}

#Adding the EOS token to end of every ans
for i in range(len(clean_answers)):
    clean_answers[i] += ' <EOS>'

#Translation of text into int and replacing words with their integers
questions_into_int = []
for question in clean_questions:
    ints = []
    for word in question.split():
...
# Placeholders for inpits and the targets
def model_inputs():
...
def decoder_rnn(decoder_embbeded_input, decoder_embeddings_matrix, encoder_state, num_words, sequence_length, rnn_size, num_layers, word2int, keep_prob, batch_size):
    with tf.variable_scope("decoding") as decoding_scope:
        ...
        training_predictions = decode_training_set(encoder_state, decoder_cell, decoder_embbeded_input, sequence_length, decoding_scope, output_function, keep_prob, batch_size)
...
############################# Part 3: Trainging the seq2seq model ################################
...
# Splitting the data into batches of q and a
...
# Splitting the q and a into training and validation sets
        ...
        if batch_index % batch_index_check_validation_loss == 0 and batch_index > 0:
            ...
            for batch_index_validation, (padded_questions_in_batch, padded_answers_in_batch) in enumerate(split_into_batches(validation_questions, validation_answers, batch_size)):
                ...
                if average_validation_loss_error <= min(list_validation_loss_error):
                    ...
                    print("Sorry I did not learn much, I need to get better.")
                    early_stopping_check += 1
                    if early_stopping_check == early_stopping_stop:
                        break
    if early_stopping_check == early_stopping_stop:
        print("My apologies, but this is the best I can do.")
        break
print("Game Over")
